...
api_key = "zt1NjvGXf22YpZsLUkNZAy9fCULjW6iO"
...
def compare_travel_options(flight_price_per_person: float, car_price_per_person: float):
 ...
 # Feed the email body as content to the Mistral API
chat_response = client.chat.complete(
    model=model,
    messages=[
        {
            "role": "user",
            "content": email_data.body,  # Use the extracted body as content
        },
    ]
)

# Print the response from Mistral
Recomm=chat_response.choices[0].message.content
...
def main():
    ...
    with tab6:
        st.write(Recomm)  # Display the response generated by Mistral
...
def send_email_with_flight_details(flight_data):
    ...
    msg['Subject'] = f"Your travel recommendation"
    ...
    Here is the travel itinerary for your trip:
    
    
    
    {Recomm}
    
    Thank you for using our travel search service! For more options visit https://travelassistant4.streamlit.app/    
    Best regards,
